...
function get_comment_reply_link( $args = array(), $comment = null, $post = null ) {
	...
	if ( get_option( 'comment_registration' ) && ! is_user_logged_in() ) {
		$link = sprintf( '<a rel="nofollow" class="comment-reply-login button blackbutton" href="%s">%s</a>',
			esc_url( reset_page_url_param('?refer',curPageURL(),wc_customer_login_url()).'#'.$args['respond_id']),
			$args['login_text']
		);
	} else {
		...
		$link = sprintf( "<a rel='nofollow' class='button blackbutton comment-reply-link' href='%s' onclick='%s' aria-label='%s'>%s</a>",
...
function comment_form( $args = array(), $post_id = null ) {
	...
	$defaults = array(
		...
		'logged_in_as'         => '<p class="logged-in-as">' . sprintf(
		                              ...
		                              __( '<a href="%1$s" aria-label="%2$s">Logged in as %3$s</a>. <a href="%4$s" class="pblue pbold">Log out?</a>' ),
		                              get_edit_user_link(),
