...
<!--      <p id="--><?php //echo ($user=='add-new-user') ? 'wpua-add-button' : 'wpua-add-button-existing'?><!--"><button type="button" class="button" id="--><?php //echo ($user=='add-new-user') ? 'wpua-add' : 'wpua-add-existing'?><!--" name="--><?php //echo ($user=='add-new-user') ? 'wpua-add' : 'wpua-add-existing'?><!--" data-title="--><?php //_e('Choose Image','wp-user-avatar'); ?><!--: --><?php //echo $user->display_name; ?><!--">--><?php //_e('Choose Image','wp-user-avatar'); ?><!--</button></p>-->
    <?php //elseif(!$wp_user_avatar->wpua_is_author_or_above()) : // Upload button ?>
      <p id="<?php echo ($user=='add-new-user') ? 'wpua-upload-button' : 'wpua-upload-button-existing'?>" class="woocommerce-FormRow woocommerce-FormRow--wide form-row form-row-wide">
        <label><?php _e('Avatar');?></label>
        <span class="woocommerce-Input">
      ...
      <p id="<?php echo ($user=='add-new-user') ? 'wpua-upload-messages' : 'wpua-upload-messages-existing'?>" class="woocommerce-FormRow woocommerce-FormRow--wide form-row form-row-wide">
        <label><?php _e('Photo');?></label>
        <span id="<?php echo ($user=='add-new-user') ? 'wpua-max-upload' : 'wpua-max-upload-existing'?>" class="description"><?php printf(__('Maximum upload file size: %d%s.','wp-user-avatar'), esc_html($wpua_upload_size_limit_with_units), esc_html('KB, 300px*300px')); ?></span>
        <span id="<?php echo ($user=='add-new-user') ? 'wpua-allowed-files' : 'wpua-allowed-files-existing'?>" class="description"><?php _e('Allowed Files','wp-user-avatar'); ?>: <?php _e('<code>jpg jpeg png gif svg</code>','wp-user-avatar'); ?></span>
      </p>
    <?php //endif; ?>
      <div class="woocommerce-FormRow woocommerce-FormRow--wide form-row form-row-wide">
      <div id="<?php echo ($user=='add-new-user') ? 'wpua-images' : 'wpua-images-existing'?>" class="<?php echo $hide_images; ?> woocommerce-Input">
...
<!--        </p>-->
        ...
        <div class="clearbox">
          <p>
            <span id="<?php echo ($user=='add-new-user') ? 'wpua-remove-button' : 'wpua-remove-button-existing'?>" class="<?php echo $hide_remove; ?>">
...
<!--            <span id="<?php /*echo ($user=='add-new-user') ? 'wpua-undo-button' : 'wpua-undo-button-existing'*/?>"><button type="button" class="button" id="<?php /*echo ($user=='add-new-user') ? 'wpua-undo' : 'wpua-undo-existing'*/?>" name="wpua-undo"><?php /*_e('Undo','wp-user-avatar'); */?></button></span>
-->          </p>

        </div>
      </div>
    </div>
      <p id="user-description" class="woocommerce-FormRow woocommerce-FormRow--wide form-row form-row-wide">
          <label><?php _e('Description')?></label>
          <textarea name="description" id="" cols="30" rows="10" ><?php echo  $current_user->description ?></textarea>
      </p>
      <div class="woocommerce-FormRow woocommerce-FormRow--wide form-row form-row-wide">
          <div class="woocommerce-Input">
              <?php submit_button(__('Save','wp-user-avatar')); ?>
          </div>
      </div>
  ...
  public static function wpua_upload_errors($errors, $update, $user) {
    ...
    if($update && !empty($_FILES['wpua-file'])) {
      ...
      if(!empty($type) && !preg_match('/(jpe?g|gif|png|svg)$/i', $type)) {
  ...
  public static function wpua_action_process_option_update($user_id) {
    ...
    $wpua_user_descirption = $_POST['description'];
    update_user_meta($user_id,'description',$wpua_user_descirption);
    if($wp_user_avatar->wpua_is_author_or_above()) {
