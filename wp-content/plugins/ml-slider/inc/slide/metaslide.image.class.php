...
class MetaImageSlide extends MetaSlide {
    ...
    private function get_flex_slider_markup( $slide )
    {
        ...
        $image_attributes = array(
            // 'src' => $this->settings['home_banner'] == 'true' ? '' : $slide['src'],
            'src' => $slide['src'],
            'height' => $slide['height'],
            'width' => $slide['width'],
            'alt' => $slide['alt'],
        ...
        if($this->settings['home_banner'] == 'true'){
            // $backGroundImage = "display: none; width: 100%;background:url(".$slide['src'].") no-repeat center center / cover;";
            $backGroundImage = "display: none; width: 100%;";
        }
