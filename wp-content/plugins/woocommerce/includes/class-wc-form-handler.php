...
class WC_Form_Handler {
	...
	public static function init() {
		...
		add_action( 'wp_loaded', array( __CLASS__, 'process_user_video' ), 20 );
		add_action( 'wp_loaded', array( __CLASS__, 'process_user_tutorial' ), 20 );
		add_action( 'wp_loaded', array( __CLASS__, 'process_user_contest' ), 20 );
	...
	public function process_user_video(){
	    if(!empty($_POST['inventions'])){
          try{
             if(insert_customer_video($_POST)){
                 wp_safe_redirect(wp_get_referer() ? wp_get_referer() : wc_get_account_endpoint_url( 'inventions') );
                 exit;
             }else{
                 wc_add_notice( '<strong>' . __( 'Error', 'woocommerce' ) . ':</strong> There were some mistakes in the submitted data' , 'error' );
             };
          }catch ( Exception $e ) {
              wc_add_notice( '<strong>' . __( 'Error', 'woocommerce' ) . ':</strong> ' . $e->getMessage(), 'error' );
          }
    ...
    public function process_user_tutorial(){
        if(!empty($_POST['tutorial'])){
            try{
                if($post_id = insert_customer_tutorial($_POST)){
                    wp_safe_redirect(wp_get_referer() ? wp_get_referer() : reset_page_url_param('post_id',$post_id,wc_get_account_endpoint_url( 'tutorial-view')));
                    exit;
                }else{
                    wc_add_notice( '<strong>' . __( 'Error', 'woocommerce' ) . ':</strong> There were some mistakes in the submitted data' , 'error' );
                };
    ...
    public function process_user_contest(){
        if(!empty($_POST['contest'])){
            try{
                if($post_id = insert_customer_contest($_POST)){
                    wp_safe_redirect(wp_get_referer() ? wp_get_referer() : wc_get_account_endpoint_url( 'contest'));
                    exit;
                }else{
                    wc_add_notice( '<strong>' . __( 'Error', 'woocommerce' ) . ':</strong> There were some mistakes in the submitted data' , 'error' );
                };
