...
function plugin_activation_addfield() {
    global $wpdb;
    $table = $wpdb->prefix. 'huge_it_videogallery_galleries';
    $video_table = $wpdb->prefix . 'huge_it_videogallery_videos';
    $sql  = "ALTER TABLE {$table} ADD `video_type` VARCHAR(10) NOT NULL DEFAULT 'official';";
    $sql .= "ALTER TABLE {$video_table } ADD `create_date` TIMESTAMP ;";
    $wpdb->query($sql);
}
//register_activation_hook(__FILE__, 'plugin_activation_addfield');
