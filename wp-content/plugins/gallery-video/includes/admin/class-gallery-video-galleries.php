...
GROUP BY " . $wpdb->prefix . "huge_it_videogallery_videos.videogallery_id ";
	...
	public function edit_video_gallery( $id ) {
        ...
        $query_string = null;
        if($published) {
            $query_string = " AND published={$published}";
        }
		$query                          = $wpdb->prepare( "SELECT * FROM " . $wpdb->prefix . "huge_it_videogallery_videos where videogallery_id = %d {$query_string} order by ordering ASC  ", $row->id );
        $all_query                      = $wpdb->prepare( "SELECT * FROM " . $wpdb->prefix . "huge_it_videogallery_videos where videogallery_id = %d  order by ordering ASC  ", $row->id );
        $pub_query                      = $wpdb->prepare( "SELECT * FROM " . $wpdb->prefix . "huge_it_videogallery_videos where videogallery_id = %d  AND published = 1 order by ordering ASC  ", $row->id );
        $ped_query                      = $wpdb->prepare( "SELECT * FROM " . $wpdb->prefix . "huge_it_videogallery_videos where videogallery_id = %d  AND published = 2  order by ordering ASC  ", $row->id );
        $rsa_query                      = $wpdb->prepare( "SELECT * FROM " . $wpdb->prefix . "huge_it_videogallery_videos where videogallery_id = %d  AND published = 3 order by ordering ASC  ", $row->id );
        $rowim                          = $wpdb->get_results( $query );
        $all_rowim                      = $wpdb->get_results($all_query);
        $pub_rowim                      = $wpdb->get_results($pub_query);
        $ped_rowim                      = $wpdb->get_results($ped_query);
        $rsa_rowim                      = $wpdb->get_results($rsa_query);
	...
	function save_video_gallery_data( $id ) {
        ...
        $query_string = null;
        if($_GET['published']) {
            $query_string = " AND published={$_GET['published']}";
        }
		$query = $wpdb->prepare( "SELECT * FROM " . $wpdb->prefix . "huge_it_videogallery_galleries WHERE id = %d", $id );
		$row   = $wpdb->get_row( $query );
		$query = $wpdb->prepare( "SELECT * FROM " . $wpdb->prefix . "huge_it_videogallery_videos where videogallery_id = %d  {$query_string} order by id ASC", $row->id );
		$rowim = $wpdb->get_results( $query );
		...
		foreach ( $rowim as $key => $rowimages ) {
			if ( isset( $_POST[ "order_by_" . $rowimages->id . "" ] ) ) {
				...
				$published     = sanitize_text_field( $_POST[ 'published'. $rowimages->id ]);
